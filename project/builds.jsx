// builds gallery — all builds page
...
function GridBuildCard({ b, onClick }) {
  ...
  return (
    ...
    >
      ...
      <div style={{
        font: "400 9px var(--kw-mono)", color: KW.text4,
        marginTop: -2,
      }}>by {b.by}</div>
      <button style={{
        marginTop: "auto",
...
function BuildsPage() {
  ...
  const onSearch = () => { if (q.trim()) flashToast(`searched: "${q}"`); };

  // visible list — filter by name match in q (purely visual)
  const list = ALL_BUILDS.filter(b => !q || b.name.toLowerCase().includes(q.toLowerCase()));

  return (
    <div data-screen-label="02 builds gallery" style={{
      ...
      <div style={{ flex: 1, padding: "32px 24px 32px" }}>
        ...
        <div style={{
          ...
          <div>
            ...
            <div style={{
              font: "400 12px var(--kw-mono)", color: KW.text3, marginTop: 8,
            }}>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
          ...
          <SortButton value={sort} onClick={()=>flashToast("→ sort menu")} />
        </div>
        ...
        <div style={{
          ...
          {list.map((b,i) => <GridBuildCard key={i} b={b} onClick={()=>flashToast(`→ ${b.name}`)} />)}
        </div>
...
Object.assign(window, { BuildsPage });
