...
export default function HomePage() {
  ...
  const onSearch = () => {
    if (!q.trim()) return;
    navigate(`/builds?q=${encodeURIComponent(q)}`);
  };
    ...
    <div style={{
      ...
      <div style={{ flex: 1, padding: "0 24px 32px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 36 }}>
          {FEATURED.map((b, i) => (
            <FeaturedCard key={i} b={b} onClick={() => navigate(`/builds/${b.name.toLowerCase().replace(/\s+/g, "-")}`)} />
          ))}
        </div>
        ...
        <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 36 }}>
          {FILTER_PILLS.map(p => (
            <Pill key={p} active={filter === p} onClick={() => {
              setFilter(p);
              navigate(p === "all" ? "/builds" : `/builds?filter=${encodeURIComponent(p)}`);
            }}>{p}</Pill>
          ))}
        </div>
        ...
        <SectionHeader
          ...
          right={
            <a
              ...
              style={{ font: "400 11px var(--kw-mono)", color: KW.lavender, cursor: "pointer", textDecoration: "none" }}
            >view all →</a>
          }
        />
        <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 10, marginBottom: 36 }}>
          {RECENT.map((b, i) => (
            <RecentCard key={i} b={b} onClick={() => navigate(`/builds/${b.name.toLowerCase().replace(/\s+/g, "-")}`)} />
          ))}
        </div>
